...
function isEmailUnique(email, customerId = null) {
  ...
  return !customers.some(
    (c) => c.email.toLowerCase() === email.toLowerCase() && c.id !== customerId
  );
}
...
function openEditCustomerModal(customerId) {
  ...
  document.getElementById("edit-customer-spending").value =
...
function updateCustomer() {
  ...
  const totalSpending =
    parseFloat(document.getElementById("edit-customer-spending").value) || 0;
...
function deleteSelectedCustomers() {
  const selectedCheckboxes = document.querySelectorAll(
    ".customer-row .select-customer:checked"
  );
  ...
  const updatedCustomers = customers.filter(
    (c) => !customerIdsToDelete.includes(c.id)
  );
  saveCustomersToLocal(updatedCustomers);
...
function filterAndSearchCustomers() {
  ...
  const filteredCustomers = customers.filter((customer) => {
    ...
    if (filterJoinDate) {
      ...
      if (
        filterJoinDate === "last-month" &&
        (customerDate < firstDayLastMonth || customerDate >= firstDayThisMonth)
      ) {
        isValid = false;
      }
      ...
      if (
        filterJoinDate === "last-year" &&
        (customerDate < firstDayLastYear || customerDate >= firstDayThisYear)
      ) {
        isValid = false;
      }
    }
    ...
    if (filterSpending) {
      ...
      if (
        filterSpending === "0-1000000" &&
        (spending < 0 || spending > 1000000)
      ) {
        isValid = false;
      }
      if (
        filterSpending === "1000000-5000000" &&
        (spending < 1000000 || spending > 5000000)
      ) {
        isValid = false;
      }
      if (
        filterSpending === "5000000-10000000" &&
        (spending < 5000000 || spending > 10000000)
      ) {
        isValid = false;
      }
...
//HÀM ĐẾM VÀ TÍNH TOÁN

//Hàm đếm lịch sử đơn hàng của một khách hàng
function countCustomerOrdersByEmail(customerEmail) {
  // Kiểm tra đầu vào
  if (!customerEmail) {
    console.error("Vui lòng cung cấp email khách hàng.");
    return [];
  }

  let orderIds = [];
  const customerOrders = orderList.filter((order) =>
    order.customer.some(
      (c) => c.emailCus.toLowerCase() === customerEmail.toLowerCase()
    )
  );

  // Lấy ID của các đơn hàng
...
document
  .querySelector(".search-input")
  .addEventListener("input", filterAndSearchCustomers);
document
  .getElementById("filter-name")
  .addEventListener("input", filterAndSearchCustomers);
document
...
document
  ...
  .addEventListener("input", filterAndSearchCustomers);
document
  .getElementById("filter-join-date")
  .addEventListener("change", filterAndSearchCustomers);
document
  .getElementById("filter-spending")
  .addEventListener("change", filterAndSearchCustomers);
document
...
document
  .querySelector(".submit-customer-btn")
...
document
  .querySelector(".update-customer-btn")
...
document
  ...
  .addEventListener("click", deleteSelectedCustomers);
